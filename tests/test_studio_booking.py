...
async def test_studio_workflow(client: AsyncClient) -> None:
    ...
    studio_res = await client.post("/studios",
                                   json={"name": "Metcon Central", "timezone": "America/New_York", "cancellation_cutoff_hours": 4},
    ...
    class_res = await client.post(f"/studios/{studio_id}/classes", json={
        "name": "Savage Shred", "instructor": "Coach Dan", "start_time": "2026-10-10T12:00:00Z", "total_capacity": 1, "credit_cost": 2
    }, headers={"X-User-Id": staff_id})
    class_id = class_res.json()["id"]

    await client.post(f"/users/{member_id}/credit-packs",
                      json={"total_credits": 10, "expiry_date": "2026-11-11T00:00:00Z"}, headers={"X-User-Id": staff_id})
